...
49 项测试覆盖链接校验、账号一致性、发布去重、断点恢复、关注按钮识别、环境切换、普通 Chrome 启动、X 长文章双工具栏按钮识别，以及并发上限、排队、独立停止、失败隔离、共享记录、重复账号拦截，以及扩展鉴权、工作标签页隔离和真实 Playwright 协议接入（Chrome API 使用模拟适配器）。GitHub Actions 在 Windows / Node.js 22、24 上运行测试。
