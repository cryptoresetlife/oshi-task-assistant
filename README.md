...
- 并行扫描每个环境默认最多勾选 60 项，其余任务保留到后续轮次；启动错误同时显示在并行面板。
- 点赞点击和结果确认最多等待 30 秒；超时先读取当前点赞状态，最多重试一次，避免取消已有点赞。
- 回复框最多等待 30 秒；新回复的原帖关系最多等待 20 秒加载，仍无法确认则保留链接并暂停，不重复发帖。
- 空闲时可重新连接当前环境，恢复已关闭的工作标签页。
- 导航最多等待 45 秒，超时重载一次；明确的单条任务问题留待处理并继续后续任务，连续三项失败则暂停环境。发送结果不确定、账号不一致和验证要求仍会暂停。
- 界面显示后台实际版本和待重启状态，可在空闲时关闭助手服务；启动器检测已有服务，避免打开多个版本。
- 每个环境提供“全不选”和“选择本轮任务”，便于分批和恢复。

升级后需重启助手服务；只刷新网页不会更新后台代码。保留原目录的 `data/history.json`，重连环境并重新扫描后继续。Chrome 扩展未改变，无需重装。

已通过 65 项自动化测试及三个真实环境的并行回归：六项任务中四项获官网确认完成；两条旧回复的原帖不可用，保留链接并继续其他任务。外部网站的不可用帖子、账号验证和网络故障仍需按提示处理。

## v0.1.8 修复

扩展模式的关注、点赞和转帖提交：若官网要求先打开任务入口，助手会点击真实入口，等待官网启用完成按钮，再提交并核对完成列表。不会修改按钮禁用状态。官网入口可能额外打开一个 X 页面。扩展文件未改变，已有扩展无需重装；助手重启后重新获取连接码并连接即可。
...
65 项测试覆盖链接校验、账号一致性、发布去重、断点恢复、关注按钮识别、环境切换、普通 Chrome 启动、X 长文章双工具栏按钮识别，以及并发上限、排队、独立停止、失败隔离、共享记录、重复账号拦截，以及扩展鉴权、工作标签页隔离和真实 Playwright 协议接入（Chrome API 使用模拟适配器）。GitHub Actions 在 Windows / Node.js 22、24 上运行测试。
