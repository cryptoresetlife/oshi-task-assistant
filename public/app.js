...
function renderTasks(){
  ...
  for(const task of state.tasks){const item=node('div',undefined,'task-item');item.dataset.key=task.key;const head=node('div',undefined,'task-item-head');const check=node('input');check.type='checkbox';check.checked=selected.has(task.key);check.disabled=task.type==='manual'||state.busy||state.batch?.busy;check.setAttribute('aria-label','选择 '+task.title+' '+task.url);check.addEventListener('change',()=>{check.checked?selected.add(task.key):selected.delete(task.key);updateSelection();});const body=node('div',undefined,'task-body'),title=node('div',undefined,'task-title');title.append(node('span',task.title),node('span',labels[task.type],'tag'));const a=node('a',task.url,'task-link');a.href=task.url;a.target='_blank';a.rel='noreferrer';body.append(title,a);
    if(task.type==='reply'&&mode()==='manual'){const input=node('textarea');input.rows=2;input.placeholder='本条回复（留空使用左侧默认内容）';input.value=drafts[task.key]||'';input.disabled=state.busy||state.batch?.busy;input.setAttribute('aria-label','回复 '+task.url);input.addEventListener('input',()=>drafts[task.key]=input.value);body.append(input);}
...
function renderRecords(){const sig=JSON.stringify(state.records);if(sig===recordSignature)return;recordSignature=sig;const records=$('records');records.replaceChildren();if(!state.records.length)records.append(node('p','暂无执行记录。','muted'));
  ...
  if(r.replyUrl&&r.stage!=='done'){const resume=node('button','只续交此链接');resume.disabled=state.busy||state.batch?.busy;resume.onclick=()=>action(async()=>apply(await api('resume',{key:r.task})));result.append(node('br'),resume);}
...
function apply(s){const changedBusy=state.busy!==s.busy||state.batch?.busy!==s.batch?.busy;
  ...
  const logs=$('logs');logs.replaceChildren();for(const event of s.events){const line=node('div',undefined,'log-line');line.append(node('time',event.time),node('span',event.message));logs.append(line);}logs.scrollTop=logs.scrollHeight;if(typeof renderBatch==='function')renderBatch();
}
async function loadProfiles(wanted=$('profile').value||state.profile?.id){const ps=await api('profiles');availableProfiles=ps;if(typeof renderBatchSources==='function')renderBatchSources(ps);$('profile').replaceChildren();const placeholder=node('option','请选择已启动的环境');placeholder.value='';$('profile').append(placeholder);for(const p of ps){const o=node('option',`${p.name} · ${p.running?'已启动':'未启动'}${p.debugPort?' · '+p.debugPort:''}`);o.value=p.id;o.disabled=!p.running;$('profile').append(o);}const chosen=ps.find(p=>p.id===wanted&&p.running)||ps.find(p=>p.running);$('profile').value=chosen?chosen.id:'';if(!ps.length)placeholder.textContent='请先启动普通 Chrome 或多开环境';connectionControls();updateSelection();}
