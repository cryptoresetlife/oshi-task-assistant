...
function renderBatch(){
  ...
  for(const w of b.workers){
    ...
    if(!card){
      ...
      const tasks=node('details'),summary=node('summary'),list=node('div',undefined,'batch-task-list');tasks.append(summary,list);
      ...
      root.append(title,status,error,tasks,review,stop,records,logs);$('batch-workers').append(root);
      card={root,title,status,error,tasks,summary,list,review,target,input,approve,stop,logText,recordList,recordSig:'',selected:new Set(),seen:new Set(),taskSig:'',pendingKey:''};batchCards.set(w.id,card);
    }
    ...
    const eligible=w.tasks.filter(t=>t.type!=='manual');for(const task of eligible)if(!card.seen.has(task.key)){card.seen.add(task.key);card.selected.add(task.key);}
    for(const key of card.selected)if(!eligible.some(t=>t.key===key))card.selected.delete(key);
    const sig=JSON.stringify(w.tasks)+locked+w.disabled;if(sig!==card.taskSig){card.taskSig=sig;card.list.replaceChildren();
      for(const t of w.tasks){const label=node('label',undefined,'check'),check=node('input');check.type='checkbox';check.checked=card.selected.has(t.key);check.disabled=locked||w.disabled||t.type==='manual';check.setAttribute('aria-label',w.name+' '+t.title+' '+t.url);check.onchange=()=>{check.checked?card.selected.add(t.key):card.selected.delete(t.key);card.summary.textContent=`已选 ${card.selected.size} / ${eligible.length} 项可运行任务`;};const link=node('a',t.title+' · '+t.url);link.href=t.url;link.target='_blank';link.rel='noreferrer';label.append(check,link);card.list.append(label);}
    }
...
$('batch-prepare').onclick=()=>action(async()=>{const ids=batchSources.filter(p=>p.running&&batchChosen.has(p.id)).map(p=>p.id);if(!ids.length)throw new Error('请先勾选要并行运行的环境');const next=await api('batch/prepare',{ids,limit:Number($('batch-limit').value)});batchCards.clear();$('batch-workers').replaceChildren();batchApplySnapshot(next);});
$('batch-run').onclick=()=>action(async()=>{const plans=state.batch.workers.filter(w=>!w.disabled&&w.profile).map(w=>({id:w.id,keys:[...(batchCards.get(w.id)?.selected||[])]})).filter(p=>p.keys.length);if(!plans.length)throw new Error('请先扫描环境并勾选任务');savePrefs();batchApplySnapshot(await api('batch/start',{plans,settings:settings(),limit:Number($('batch-limit').value)}));});
